...
if [ "${NUM_EVAL_EPISODES}" -eq 0 ]; then
	echo "[run_eval] ckpt=${MAZE_CKPT_NAME} episodes=all (full epoch) use_class=${USE_CLASS_VALUES[*]}"
else
	echo "[run_eval] ckpt=${MAZE_CKPT_NAME} episodes=${NUM_EVAL_EPISODES} use_class=${USE_CLASS_VALUES[*]}"
fi
		...
		for algo in "${MAZE_ALGOS[@]}"; do
			for use_class in "${USE_CLASS_VALUES[@]}"; do
				...
				if [ "${code}" -eq 0 ]; then
					...
					echo "[run_eval] seed=${seed} dataset=${dataset}" \
						"algo=${algo} use_class=${use_class}: exited abnormally (exit ${code}); continuing."
				fi
			done
		done
	done
